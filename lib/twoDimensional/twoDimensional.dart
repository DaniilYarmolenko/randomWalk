...
class TwoDimensional extends StatefulWidget {
  TwoDimensional({Key key}) : super(key: key);

  @override
  _TwoDimensionalState createState() => _TwoDimensionalState();
}

class _TwoDimensionalState extends State<TwoDimensional> {
          ...
          Container(
            ...
            GridPaper(
              child: Container(
                ...
                height: 0.5 * MediaQuery.of(context).size.height - 10,
                child: CustomPaint(
                  painter: OpenPainter(),
                ),
              ),
              interval: ((MediaQuery.of(context).size.width - 20)) * 10 / 100,
              // subdivisions: 1,
              color: Colors.black,
            ),
          ])),
  ...
  void firstDimensional() {
    ...
    fd(timeNumberStep, sellNumberX, quantityTimeStep, dx, timeStep, velocity,
        porEf, al, mass);
    rWDouble(quantityTimeStep, timeStep, timeNumberStep, velocity, al, porEf,
        mass, sellNumberX, dx, numPar, m);
  }
      ...
      for (var i = 1; i < quantityTimeStep; i++) {
        ...
        for (var j = 0; j < sellNumberX; j++) {
          ...
          for (var k = 0; k < nPas; k++) {
            ...
            // OpenPainter.points.add(Offset(datX[i][j][k], 100));
          } // цикл частиц
...
void rWDouble(quantityTimeStep, timeStep, timeNumberStep, velocity, al, porEf,
    mass, sellNumberX, dx, nPas, m) {
  var dy = 4;
  var startx = 0;
  var starty = 0;
  var sellNumberY = 10;
  var u = velocity / porEf;
  ...
  var datY = List.generate(
      quantityTimeStep,
      (t) =>
          List.generate(sellNumberY, (y) => List.generate(nPas, (p) => 0.0)));
  for (var i = 1; i < quantityTimeStep; i++) {
    // print('$cRw  $cAn');
    time = time + timeStep; // действующее время
    x = 0.5 * dx;
    y = 0.5 * dy; // координата центра первой ячейки
    // xU = time * u; //- фактическое растояние пройденное
    for (var j = 0; j < sellNumberX; j++) {
      nPasVarX = 0;
      nPasVarY = 0;
        ...
        datX[i][j][k] = datX[i - 1][j][k] +
            ...
            z *
                sqrt(2 *
                    ...
                    timeStep *
                    u); // частица со временем i, ячейкой j, и номером k
        if (datX[i][j][k] < x + 0.5 * (dx) && datX[i][j][k] > x - 0.5 * (dx)) {
          nPasVarX++;
        }

        z = 2 * random.nextDouble() - 1; // рандом
        // z = random.nextDouble();
        datY[i][j][k] = datY[i - 1][j][k] +
            u * timeStep +
            z *
                sqrt(2 *
                    al *
                    timeStep *
                    u); // частица со временем i, ячейкой j, и номером k
        if (datY[i][j][k] < x + 0.5 * (dy) && datY[i][j][k] > x - 0.5 * (dy)) {
          nPasVarY++;
        }
        OpenPainter.points.add(Offset(datX[i][j][k], datY[i][j][k]));
      }
      // OpenPainter.points.add(Offset(datX[i][j][k], datY[i][j][k]));  // цикл частиц

      // if (x < time * u) {
      cRw = mass * (nPasVarX + nPasVarY) / (nPas * m * porEf * dx);
    }
  }
}
